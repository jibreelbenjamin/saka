...
<x-app :page='$page'>
    <div class="p-2 sm:p-5 sm:py-0 md:pt-5 space-y-5">
        <div class="p-5 space-y-4 flex flex-col bg-white border border-gray-200 shadow-2xs rounded-xl dark:bg-neutral-800 dark:border-neutral-700">
            <nav class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between" aria-label="Tabs" role="tablist" aria-orientation="horizontal">
                <div>
                    <p class="flex flex-col text-lg font-medium text-gray-900 dark:text-neutral-100">
                        Daftar {{ $page_title }}
                        <span class="text-xs font-normal text-gray-500 dark:text-neutral-500">Lihat dan kelola komponen nilai pada sistem.</span>
                    </p>
                </div>
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
                    <a href="{{ route($page.'.create') }}?type={{ $type }}" class="py-1.5 sm:py-2 px-2.5 inline-flex items-center gap-x-1.5 text-sm sm:text-xs font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700">
                ...
                <div>
                    <div class="relative">
                        ...
                        <div class="flex items-center gap-x-2">
                            <input id="search" type="text" class="flex-1 py-1 sm:py-1.5 ps-10 pe-8 block w-full bg-gray-100 border-transparent rounded-lg sm:text-sm focus:bg-white focus:ring-0 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-700 dark:border-transparent dark:text-neutral-400 dark:placeholder:text-neutral-400 dark:focus:bg-neutral-800 dark:focus:ring-neutral-600" placeholder="Cari komponen..." autocomplete="off">

                            <div class="w-48">
                                <select data-hs-select='{"hasSearch": false, "placeholder": "Filter tipe...", "toggleClasses": "hs-select-disabled:pointer-events-none hs-select-disabled:opacity-50 relative py-2 px-3 flex items-center gap-x-2 w-full cursor-pointer bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 text-gray-800 dark:text-white rounded-lg text-start text-xs hover:bg-gray-50 dark:hover:bg-neutral-700 focus:outline-hidden focus:bg-gray-50 dark:focus:bg-neutral-700", "dropdownClasses": "mt-2 max-h-72 pb-1 px-1 space-y-0.5 z-20 w-full bg-white dark:bg-neutral-900 border border-transparent rounded-lg shadow-xl overflow-hidden overflow-y-auto", "optionClasses": "hs-selected:bg-gray-100 dark:hs-selected:bg-neutral-800 py-2 px-4 w-full text-sm text-gray-800 dark:text-neutral-200 cursor-pointer hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-lg focus:outline-hidden focus:bg-gray-100 dark:focus:bg-neutral-800", "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"shrink-0 size-3.5 text-gray-500 dark:text-neutral-400\" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"}' onchange="window.location.href = '{{ route($page) }}?type=' + this.value + '&search=' + encodeURIComponent(document.getElementById('search').value)" class="hidden">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                            <thead>
                                <tr class="border-t border-gray-200 divide-x divide-gray-200 dark:border-neutral-700 dark:divide-neutral-700">
                                    <th scope="col" class="px-3 py-2.5 text-start text-sm font-normal text-gray-500 dark:text-neutral-500">Kode Komponen</th>
                                    <th scope="col" class="px-5 py-2.5 text-start text-sm font-normal text-gray-500 dark:text-neutral-500">Nama Komponen</th>
                                    <th scope="col" class="px-5 py-2.5 text-start text-sm font-normal text-gray-500 dark:text-neutral-500">Mata Pelajaran</th>
                                    <th scope="col" class="px-5 py-2.5 text-start text-sm font-normal text-gray-500 dark:text-neutral-500">Guru</th>
                                    <th scope="col" class="px-5 py-2.5 text-start text-sm font-normal text-gray-500 dark:text-neutral-500">Tahun Ajaran</th>
                                    <th scope="col" class="px-5 py-2.5 text-start text-sm font-normal text-gray-500 dark:text-neutral-500">KKM</th>
                                    <th scope="col" class="px-5 py-2.5 text-start text-sm text-gray-500 dark:text-neutral-500">Status</th>
                            ...
                            <tbody id="data-container" class="divide-y divide-gray-200 dark:divide-neutral-700">
                                ...
                                <tr class="divide-x divide-gray-200 dark:divide-neutral-700">
                                    <td class="whitespace-nowrap px-3 py-3 text-sm text-gray-700 dark:text-neutral-300">{{ $item->{$typeConfig['code']} }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-neutral-300">{{ $item->nama_komponen }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-neutral-300">{{ $item->mapel->nama_mapel ?? '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-neutral-300">{{ $item->guru->nama ?? '-' }}</td>
                                ...
                                @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-neutral-400">
                                        Tidak ada komponen nilai {{ $typeLabel }} yang ditemukan.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
...
<script>
    ...
    searchContainer.addEventListener('input', (e) => {
        ...
        if (value.length >= 1 || value.length === 0) {
            debounceTimer = setTimeout(() => {
                ...
                window.location.href = "{{ route($page) }}?type={{ $type }}&search=" + encodeURIComponent(value);
            }, 700);
        }
    });
    ...
    if (searchValue) {
        searchContainer.value = searchValue;
    }
</script>
